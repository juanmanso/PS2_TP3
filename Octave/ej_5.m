...
B2=double(subs(numB,{b,m,k},[b_const m_const 0.05]));
A2=double(subs(denA,{b,m,k},[b_const m_const 0.05]));
...
result=solve(w(1:2,end)'==numB,w(3:4,end)'==denA(2:3),m_const==m,[m k b]);
...
%axis([largo_parte_fir + largo_parte_iir, largo_senial_entrada, -0.3, 0.7]);
title('Convergencia De Coeficientes NLMS - Corte de resorte');
setGraphSize(fig1,wide_1);
leg1=legend({'$\hat{B1}$','$\hat{B2}$','$\hat{A2}$','$\hat{A3}$','$B1$','$B2$','$A2$','$A3$'});
set(leg1,'Interpreter','latex');
leg1.Location='eastoutside';
